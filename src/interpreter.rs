...
fn execute_List(
    functionname: String,
    ls: List,
    state: &mut HashMap<String, hashstate>,
    idmap: &mut HashMap<i32, i32>,
    addressmap: &mut HashMap<i32, hashdata>,
    currentid: &mut i32,
) -> List {
    match ls {
        List::paran(v) => {
        ...
        List::func(fu) => {
            ...
            let _func_var = match fu.clone() {
                ...
                function::parameters_call(v, _w) => {
                    ...
                    match fnst {
                        hashstate::state(ft, _hv, _fu, _ln) => match unbox(ft.clone()) {
...
fn changeFunctionState(
    ...
    match oldstate.clone() {
        hashstate::state(_fs, hv, fu, ln) => {
            changeState(
                function.clone(),
                hashstate::state(Box::new(st), hv, fu, ln),
                state,
            );
        }
            ...
            match f.clone() {
                function::parameters_def(na, ar, ty, ele) => {
                    ...
                    changeState(
                        tempstring,
                        hashstate::state(
                            Box::new(functionstate::Declared),
                            Box::new(temp),
                            Box::new(f.clone()),
                            -1,
                        ),
    ...
    match func_var {
        function::parameters_def(na, _m, _n, _o) => {
            if unbox(na.clone()) == "main" {
                //Special case for main. Calls in when we find define in code. Ensures that it is always called.
                changeFunctionState(unbox(na.clone()), functionstate::Running, state); //Set new as running

                let w = function_arguments_call::variable(Box::new(variable::name(Box::new(
                    "".to_string(),
                ))));
                paramcall_execute(
                    na.clone(),
        ...
        function::parameters_call(v, w) => {
            ...
            changeFunctionState(functionname.clone(), functionstate::Running, state);
            //Set old function as running again
        }
    };
}
...
fn var_execute(
    ...
    match variable_var {
        variable::parameters(na, ty, val) => {
            let testIfExist =
                getLocalVariable(unbox(functionname.clone()), unbox(na.clone()), state);
            match testIfExist {
                hashvariable::var(_oldname, oldaddress) => {
                    //If we get this it means a local var exists with same name, replace val in memory.
                    match unbox(val) {
                        //This means we don't have to touch local var, since it just contains address.
                        variable_value::Boolean(b) => {
                            //And we replaced value at address
                            if ty != Type::boolean {
                                panic!("Type mismatch in var: var_execute")
                            };
                            let temp = hashdata::valuebool(b);
                            let _addressOfTemp =
                                replaceAtMemory(oldaddress, temp, idmap, addressmap);
                        }
                        ...
                        variable_value::boxs(b) => {
                            ...
                            match ls.clone() {
                                ...
                                List::var(v) => {
                                    ...
                                    match varval.clone() {
                                        List::Num(n) => {
                                            ...
                                            let temp = hashdata::valuei32(n);
                                            let _addressOfTemp = replaceAtMemory(
                                                oldaddress, temp, idmap, addressmap,
                                            );
                                        }
                                        List::boolean(b) => {
                                            if ty != Type::boolean {
                                ...
                                List::Cons(lli, op, rli) => {
                                    let val = cons_execute(
                                        unbox(functionname.clone()),
                                        lli,
                                        op,
                                        rli,
                                        state,
                        ...
                        variable_value::boxs(b) => {
                            ...
                            match ls.clone() {
                                List::Num(n) => {
                                    ...
                                    let temp = hashdata::valuei32(n);
                                    let addressOfTemp =
                                        addToMemory(0, temp, idmap, addressmap, currentid);
                                    let temp2 = hashvariable::var(unbox(na), addressOfTemp);
                                    let resadd =
                                        addLocalVariable(unbox(functionname), temp2, state);
                                    if resadd == false {
                                        return panic!("Adding local variable failed: var_execute");
                                    }
                                }
                                ...
                                List::var(v) => {
                                    ...
                                    match varval.clone() {
                                        List::Num(n) => {
                                            ...
                                            if resadd == false {
                                                return panic!(
                                                    "Adding local variable failed: var_execute"
                                                );
                                            }
                                        }
                                        List::boolean(b) => {
        ...
        variable::name(v) => {
            // Access local var with same name and return it.
            ...
            match lvar {
                hashvariable::var(n, a) => {
                    ...
                    match value {
                        hashdata::valuei32(va) => {
                            return List::Num(va);
                        }
                        hashdata::valuebool(va) => {
                            return List::boolean(va);
                        }
                        _ => {
                            return List::Num(0);
                        }
                ...
                _ => panic!(
                    "Name given {:?} is incorrect, not in local variables: to var_execute",
                    unbox(v.clone())
                ),
            };
        }
...
fn return_execute(
    ...
    match fnstate {
        hashstate::state(_st, vars, fu, line) => match var_val {
            variable_value::Boolean(b) => {
                let temp = hashdata::valuebool(b);
                let addressOfTemp = addToMemory(0, temp, idmap, addressmap, currentid);
                let hashdata_ret = hashdata::address(addressOfTemp);
                let temp = hashstate::state(
                    Box::new(functionstate::Returned(Box::new(hashdata_ret))),
                    vars.clone(),
                    fu.clone(),
                    line.clone(),
                );
                        ...
                        match variab {
                            hashvariable::var(_na, ad) => {
                                ...
                                match hdata {
                                    hashdata::address(_a) => {
                                        let untangledData =
                                            getFromAddressHashdata(hdata, addressmap);
                                        let temp = hashstate::state(
                                            Box::new(functionstate::Returned(Box::new(
                                                untangledData,
                                            ))),
                                            vars.clone(),
                                            fu.clone(),
                                    ...
                                    hashdata::valuebool(_va__b) => {
                                        let temp = hashstate::state(
                                            Box::new(functionstate::Returned(Box::new(hdata))),
                                            vars.clone(),
                                            fu.clone(),
                                            line.clone(),
                                        );
                                    ...
                                    _ => {
                                        panic!("This is not supposed to happen.... return_execute")
                                    }
                                };
                            }
            ...
            variable_value::boxs(va) => match unbox(va) {
                List::var(v) => {
                ...
                List::Cons(bl, opr, br) => {
                    ...
                    let hashdata_ret = match consval {
                        List::Num(n) => hashdata::valuei32(n),
                        List::boolean(n) => hashdata::valuebool(n),
                        _ => panic!(""),
                    };
                    let temp = hashstate::state(
                        Box::new(functionstate::Returned(Box::new(hashdata_ret))),
...
fn function_elements_execute(
    functionname: Box<String>,
    fe: function_elements,
    state: &mut HashMap<String, hashstate>,
    idmap: &mut HashMap<i32, i32>,
    addressmap: &mut HashMap<i32, hashdata>,
    currentid: &mut i32,
    ...
    match fe {
        function_elements::ele_list(v, w) => {
            ...
            let res1 = function_elements_execute(
                functionname.clone(),
                ele1,
                state,
                idmap,
            ...
            let res2 = function_elements_execute(
                functionname.clone(),
                ele2,
                state,
                idmap,
...
fn paramcall_execute(
    functionname: Box<String>,
    oldfunctionname: Box<String>,
    args: Box<function_arguments_call>,
    state: &mut HashMap<String, hashstate>,
    idmap: &mut HashMap<i32, i32>,
    addressmap: &mut HashMap<i32, hashdata>,
    currentid: &mut i32,
    ...
    match fnstate {
        hashstate::state(st, _v, _func, _line) => match unbox(st.clone()) {
            functionstate::Declared => function_arguments_call_execute(
                functionname.clone(),
                oldfunctionname.clone(),
                args,
                state,
                idmap,
                addressmap,
            ...
            functionstate::Running => function_arguments_call_execute(
                functionname.clone(),
                oldfunctionname.clone(),
                args,
                state,
            ...
            functionstate::Calling => function_arguments_call_execute(
                functionname.clone(),
                oldfunctionname.clone(),
                args,
                state,
            ...
            functionstate::Stopped => function_arguments_call_execute(
                functionname.clone(),
                oldfunctionname.clone(),
                args,
                state,
            ...
            functionstate::Looping => function_arguments_call_execute(
                functionname.clone(),
                oldfunctionname.clone(),
                args,
                state,
            ...
            functionstate::Returned(v) => function_arguments_call_execute(
                functionname.clone(),
                oldfunctionname.clone(),
                args,
                state,
...
fn function_arguments_call_execute(
    functionname: Box<String>,
    oldfunctionname: Box<String>,
    args: Box<function_arguments_call>,
    state: &mut HashMap<String, hashstate>,
        ...
        hashstate::state(_st, v, fu, line) => {
            ...
            match unbox(fu.clone()) {
                function::parameters_def(_na, fuag, _ty, ele) => {
                    ...
                    function_arguments_call_declare(
                        functionname.clone(),
                        oldfunctionname.clone(),
                        args,
                        functionargs,
...
fn function_arguments_call_declare(
    functionname: Box<String>,
    oldfunctionname: Box<String>,
    args: Box<function_arguments_call>,
    fuargs: Box<function_arguments>,
    state: &mut HashMap<String, hashstate>,
    idmap: &mut HashMap<i32, i32>,
    addressmap: &mut HashMap<i32, hashdata>,
    currentid: &mut i32,
    ...
    if unbox(functionname.clone()) == "main" {
        //If we are in main, declare no variables.
        return;
    }
    ...
    match temp {
        function_arguments_call::arg_call_list(a1, a2) => {
            let (varl, far) =
                match unbox(fuargs) {
                    ...
                    _ => return panic!(
                        "Too many inputs given to function {:?} : function_arguments_call_declare",
                        functionname.clone()
                    ),
                };
            let fal = Box::new(function_arguments::var(varl));
            let _leftSide = function_arguments_call_declare(
                functionname.clone(),
                oldfunctionname.clone(),
                a1,
                fal,
            ...
            let _rightSide = function_arguments_call_declare(
                functionname.clone(),
                oldfunctionname.clone(),
                a2,
                far,
        ...
        function_arguments_call::function(fu) => {
            let functionargs = match unbox(fuargs.clone()) {
                function_arguments::arg_list(le, ri) => panic!(
                    "Too few inputs given to function {:?} : function_arguments_call_declare",
                    functionname.clone()
                ),
                function_arguments::var(v) => v,
            };
        ...
        function_arguments_call::variable(va) => {
            ...
            match unbox(va.clone()) {
                ...
                variable::name(n) => {
                    let vnam = unbox(n);
                    let hvar =
                        getLocalVariable(unbox(oldfunctionname.clone()), vnam.clone(), state);
                    ...
                    let hdata = getFromAddressHashdata(ffrommem, addressmap);
                    let ty = match hdata {
                        hashdata::valuei32(_) => Type::Integer,
                        _ => Type::boolean,
                    };
                    if vartype != ty {
